...
export function NegociacaoDetalhePage() {
  ...
  if (isLoading) {
    return <PageShell><DetalheSkeleton /></PageShell>;
  }
  ...
  const percentualPago =
    negociacao.valorTotalReceber > 0
      ? Math.round((negociacao.valorPago / negociacao.valorTotalReceber) * 100)
      : 0;
  ...
  return (
    <PageShell>
      ...
      <div className="flex items-center justify-between border-b border-border bg-surface px-10 py-6">
        <div>
          ...
          <div className="mt-1 text-[13px] text-muted">
            {negociacao.numero} · {negociacao.cliente.nome} · aberta em {formatarData(negociacao.dataNegociacao)}
          </div>
        </div>
      ...
      <div className="px-10 py-8">
        {/* Os dois números "ponto de partida" */}
        ...
        {/* Bloco hierarquizado: tarifas (dedução) -> pago -> a receber (progresso) */}
        <div className="relative mb-7 overflow-hidden rounded-panel border border-border bg-surface p-[22px]">
          ...
          <div className="flex items-center justify-between border-b border-border-subtle py-4">
            ...
            <div className="tabnum font-mono text-sm text-muted-foreground">
              {formatarMoeda(negociacao.valorPago)}
            </div>
          </div>
